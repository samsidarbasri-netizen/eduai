    ...
    if not st.session_state.lkpd_list:
        ...
        if st.button("Kirim Jawaban"):
            if name and answer:
                ...
                df = pd.DataFrame(st.session_state.answers)
                df.to_csv("rekapan_nilai.csv", index=False, encoding="utf-8-sig")
...
# ====== MODE REKAP ======
    ...
    if len(st.session_state.answers) == 0:
        ...
        if os.path.exists("rekapan_nilai.csv"):
            st.download_button(
                label="📥 Unduh Data CSV",
                data=open("rekapan_nilai.csv", "rb"),
                file_name="rekapan_nilai.csv",
                mime="text/csv"
            )
    else:
        df = pd.DataFrame(st.session_state.answers)
